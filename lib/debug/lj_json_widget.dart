...
class LJJsonViewerWidget extends StatefulWidget {

  final Map<String, dynamic> jsonObj;
  final bool? notRoot;

  LJJsonViewerWidget (this.jsonObj, {this.notRoot});

  @override
  LJJsonViewerWidgetState createState() => new LJJsonViewerWidgetState();
}

class LJJsonViewerWidgetState extends State<LJJsonViewerWidget> {
  ...
  static getContentWidget(dynamic content){
    if(content is List){
      ...
      return LJJsonViewerWidget(content, notRoot: true);
    }
  }
...
class _JsonArrayViewerWidgetState extends State<JsonArrayViewerWidget> {
  ...
  _getList(){
    ...
    for(dynamic content in widget.jsonArray){
      bool ex = LJJsonViewerWidgetState.isExtensible(content);
      bool ink = LJJsonViewerWidgetState.isInkWell(content);
      list.add(
        Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            ex?(openFlag[i] ?Icon(Icons.arrow_drop_down, size: 14, color: Colors.grey[700]):Icon(Icons.arrow_right, size: 14, color: Colors.grey[700])):const Icon(Icons.arrow_right, color: Color.fromARGB(0, 0, 0, 0),size: 14,),
      ...
      list.add(const SizedBox(height: 4));
      if(openFlag[i]){
        list.add(LJJsonViewerWidgetState.getContentWidget(content));
      }
      i++;
    }
    return list;
  ...
  getValueWidget(dynamic content, int index){
    if(content == null){
      ...
      if(content.isEmpty){
        ...
        return InkWell(
          child: Text('Array<${LJJsonViewerWidgetState.getTypeName(content)}>[${content.length}]', style: TextStyle(color: Colors.grey),),
          onTap: (){
            setState(() {
              openFlag[index] = !(openFlag[index]);
            });
